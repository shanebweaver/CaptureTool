...
internal sealed class WaveAudioChunkPlan
{
        ...
        out WaveAudioChunkPlan? plan)
    {
        ...
        try
        {
            ...
            ushort blockAlignment = BinaryPrimitives.ReadUInt16LittleEndian(
                formatChunk.AsSpan(12, 2));
            uint bytesPerSecond = BinaryPrimitives.ReadUInt32LittleEndian(
                formatChunk.AsSpan(8, 4));
            ...
            for (long relativeOffset = 0;
                 relativeOffset < dataLength;
                 relativeOffset += maximumChunkBytes)
            {
        ...
        while (remaining > 0)
        {
            ...
            if (read == 0)
            {
                throw new EndOfStreamException(
                    "The wave data ended before the planned chunk boundary.");
            }
    ...
    private static byte[] CreateHeader(byte[] formatChunk, long dataLength)
    {
        ...
        {
            throw new InvalidOperationException("A wave analysis chunk exceeds the RIFF size limit.");
        }
    ...
    private static TimeSpan ToTimeSpan(long byteOffset, uint bytesPerSecond) =>
        TimeSpan.FromTicks(checked(byteOffset * TimeSpan.TicksPerSecond / bytesPerSecond));
}
