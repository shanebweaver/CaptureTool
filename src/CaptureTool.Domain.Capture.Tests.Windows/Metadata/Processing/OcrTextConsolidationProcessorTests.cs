...
{
    private OcrTextConsolidationProcessor _processor = null!;

    [TestInitialize]
    public void Setup()
    {
        _processor = new OcrTextConsolidationProcessor();
    }

    [TestMethod]
    public void ProcessorId_ShouldBeCorrectValue()
    {
        _processor.ProcessorId.Should().Be("ocr-text-consolidation");
    }

    [TestMethod]
    public void SupportedKeys_ShouldContainOcrText()
    {
        _processor.SupportedKeys.Should().ContainSingle().Which.Should().Be("ocr-text");
    }
    ...
    [TestMethod]
    public async Task ProcessAsync_ShouldCreateSingleInsight_ForIsolatedEntry()
    {
        ...
        result[0].ProcessorId.Should().Be("ocr-text-consolidation");
        result[0].Timestamp.Should().Be(10_000_000L);
        result[0].Description.Should().Be("Hello world");
    }

    [TestMethod]
    public async Task ProcessAsync_ShouldMergeConsecutiveSimilarEntries()
    {
    ...
    [TestMethod]
    public async Task ProcessAsync_ShouldNotMergeDistantEntries()
    {
    ...
    [TestMethod]
    public async Task ProcessAsync_ShouldNotMergeEntriesWithDifferentText()
    {
    ...
    [TestMethod]
    public async Task ProcessAsync_ShouldOrderInsightsByTimestamp()
    {
        var entries = new List<MetadataEntry>
        {
