...
WAVEFORMATEX* DesktopAudioSource::GetFormat() const
{
    return m_device.GetFormat();
}

void DesktopAudioSource::SetAudioCallback(AudioSampleCallback callback)
{
    m_callback = callback;
}

void DesktopAudioSource::SetEnabled(bool enabled)
{
...
bool DesktopAudioSource::IsEnabled() const
{
    return m_isEnabled;
}

bool DesktopAudioSource::Initialize()
{
    if (m_isInitialized)
    {
    ...
    // Initialize audio device in loopback mode (desktop audio)
    if (!m_device.Initialize(true, &hr))
    {
        return false;
    }
...
bool DesktopAudioSource::Start()
{
    ...
    m_isRunning = true;
    m_captureThread = std::thread(&DesktopAudioSource::CaptureThreadProc, this);
...
ULONG DesktopAudioSource::AddRef()
{
    return InterlockedIncrement(&m_ref);
}

ULONG DesktopAudioSource::Release()
{
...
void DesktopAudioSource::CaptureThreadProc()
{
...
void DesktopAudioSource::Cleanup()
{
    Stop();
}
