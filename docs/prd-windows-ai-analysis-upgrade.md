...
Prefer the newer on-device Windows AI models for OCR, selected video-frame description, and speech recognition without making successful Capture Memory metadata collection depend on Copilot+ hardware or experimental speech availability. Unsupported devices must continue producing useful metadata through existing local providers.
...
8. Windows AI Speech Recognition can transcribe both captured audio and extracted video WAV audio.
9. Speech is divided into at most 15-second WAV windows so its normalized result retains useful search timecodes.
10. Microsoft Foundry Local Whisper Tiny remains the on-device speech fallback.
...
| `speech-transcript/v1` | Experimental Windows App SDK AI Speech Recognition | Microsoft Foundry Local Whisper Tiny | Preferred off until the API stabilizes |
...
flowchart LR
    ...
    Probe -->|"Unsupported / disabled"| Fallback["Legacy OCR or Foundry Whisper"]
...
- Windows AI Speech Recognition currently requires the matching experimental Windows App SDK projection/runtime. `EnableExperimentalWindowsAi` includes that provider project only in Debug; Release uses stable packages and physically excludes its assembly and manifest. Its analyzer also retains a dedicated default-off runtime flag so it can be flighted independently during local experimentation and removed without a recipe or metadata migration.
...
- Enabling experimental Windows AI speech prefers it; unsupported devices resolve Foundry Local speech.
