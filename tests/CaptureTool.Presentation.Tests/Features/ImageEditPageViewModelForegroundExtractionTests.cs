...
[TestClass]
public sealed class ImageEditPageViewModelForegroundExtractionTests
{
    [TestMethod]
    public async Task ToggleMode_WhenConsentRefused_ShouldRemainInactive()
    {
        ...
        viewModel.IsForegroundExtractionModeActive.Should().BeFalse();
        viewModel.CanToggleForegroundExtraction.Should().BeTrue();
        service.Verify(
    ...
    [TestMethod]
    public async Task ClickingImage_ShouldApplyUndoableForegroundResult()
    {
        var service = new Mock<IImageForegroundExtractionService>();
        service.Setup(x => x.GetReadyState()).Returns(ForegroundExtractionReadyState.Ready);
        service
            ...
            .ReturnsAsync(ForegroundExtractionResult.Success(new ImageFile("foreground.png")));
        ...
        await viewModel.OnForegroundExtractionRequestedAsync(new Vector2(30, 20));

        viewModel.ImageFile!.FilePath.Should().Be("foreground.png");
        viewModel.HasUndoStack.Should().BeTrue();
        viewModel.IsForegroundExtractionModeActive.Should().BeTrue();
    ...
    public async Task ChangingModes_ShouldCancelPendingExtractionAndIgnoreItsResult()
    {
        ...
        Task extraction = viewModel.OnForegroundExtractionRequestedAsync(new Vector2(30, 20));
        viewModel.IsForegroundExtractionRunning.Should().BeTrue();

        viewModel.ToggleCropModeCommand.Execute(null);

        requestToken.IsCancellationRequested.Should().BeTrue();
        viewModel.IsForegroundExtractionRunning.Should().BeFalse();
        viewModel.IsForegroundExtractionModeActive.Should().BeFalse();
        ...
        IAiFeatureConsentDialogService? consentDialogService = null)
    {
        ...
        return new ImageEditPageViewModel(
            ...
            null,
            null,
            foregroundExtractionService,
            Mock.Of<IImageForegroundExtractionFeatureAvailability>(x =>
                x.IsImageForegroundExtractionEnabled == true));
    }
}
